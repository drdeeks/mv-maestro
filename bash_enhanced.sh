...
_esm_banner() {
    ...
    # Try to detect city from timezone
    case "$tz" in
        MST|AMST) city="Phoenix, AZ" ;;
        PST|PDT) city="San Francisco, CA" ;;
        EST|EDT) city="New York, NY" ;;
        CST|CDT) city="Chicago, IL" ;;
        MST7MDT) city="Denver, CO" ;;
        *) 
            # Fallback: try geolocation API (non-blocking)
            city=$(curl -s --max-time 1 "https://ipapi.co/" 2>/dev/null | grep -o '"city":"[^"]*"' | cut -d'"' -f4 || echo "Remote")
            ;;
    esac
    ...
    # Live system stats
    local mem_used mem_total mem_pct cpu_load disk_use uptime_str
...
# ─────────────────────────────────────────────────────────────────────────────
# MV MAESTRO HELP SYSTEM
# ─────────────────────────────────────────────────────────────────────────────
mvhelp() {
    cat <<'HELPEOF'

══ MV Maestro — Command Reference ══
    made with love by @drdeeks

  QUICK COMMANDS:
    dm              Interactive menu (all features)
    dynhelp         Dynamic core commands  
    am              Alias manager
  ...
  ARCHIVE & FILES:
    ...
    find-dupes      Find duplicate files

  SECURITY & CRYPTO:
    encrypt/decrypt Age/GPG encryption
    secret-set/get  Secure secret storage
    ssh-hardening   Audit SSH config

  CONTAINERS & CLOUD:
    docker-tui      Full Docker TUI
    ts-status       Tailscale status
    cf-dns-list     Cloudflare DNS

  DEVELOPMENT:
    py-fmt          Python formatting
    npm-check       Node.js dependencies
  ...
  UTILITIES:
    ...
    random          UUID/password generation

  SYSTEM:
    sysinfo         System overview
    sysresources    Live resources
    diskinfo        Disk usage
    myip            Network info

  Tips:
    • All commands support --help flag
    • Type 'dm' for interactive menu
    • Set CF_API_TOKEN for Cloudflare commands
    • Made with love by @drdeeks
...
Extract archive files with automatic format detection.
...
Options:
  ...
  --strip N     Strip N leading path components (not implemented)
  --list, -t    List contents without extracting

Examples:
  extract myfile.tar.gz       # Extracts to ./myfile/
  extract archive.zip         # Extracts to current dir
  extract data.tar.xz         # Extracts xz compressed tar
HELP
        return 0
    }
...
# Prompt with colors - proper bash escape sequence handling
# Restored prompt with colors and git branch (Oct 2024)
PS1='\[\033[1m\]\[\033[0;34m\]\u\[\033[0m\]@\[\033[0;36m\]\h\[\033[0m\]:\[\033[0;32m\]\w\[\033[1;33m\]$(_git_branch)\[\033[0m\]\n\[\033[0;36m\]▶\[\033[0m\] '
...
# Capture last command exit code before it gets overwritten
PROMPT_COMMAND='history -a; _LAST_EXIT=$?; export _LAST_EXIT'

# Simple exit code display
_exit_code() {
    [ "${_LAST_EXIT:-0}" -ne 0 ] && echo -n " [${_LAST_EXIT}]"
}
...
# ─────────────────────────────────────────────────────────────────────────────
# DYNAMIC CORE MODULE — auto-adapting utilities
# ─────────────────────────────────────────────────────────────────────────────
[ -f "${BASH_SOURCE[0]%/*}/modules/dynamic_core.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/dynamic_core.sh"

# ─────────────────────────────────────────────────────────────────────────────
# DYNAMIC EXTENDED MODULE — formatters, scanners, menus, PowerShell parity
# ─────────────────────────────────────────────────────────────────────────────
[ -f "${BASH_SOURCE[0]%/*}/modules/dynamic_ext.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/dynamic_ext.sh"

# ─────────────────────────────────────────────────────────────────────────────
# UNIFIED NAVIGATION MENU — consistent interface for all features
# ─────────────────────────────────────────────────────────────────────────────
[ -f "${BASH_SOURCE[0]%/*}/modules/dynamic_menu.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/dynamic_menu.sh"

# ─────────────────────────────────────────────────────────────────────────────
# ADDITIONAL MENU COMMAND IMPLEMENTATIONS
# ─────────────────────────────────────────────────────────────────────────────
[ -f "${BASH_SOURCE[0]%/*}/modules/dynamic_menu_addons.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/dynamic_menu_addons.sh"

# ─────────────────────────────────────────────────────────────────────────────
# SSH PROFILE SETUP WIZARD & TAILSCALE HELPER
# ─────────────────────────────────────────────────────────────────────────────
[ -f "${BASH_SOURCE[0]%/*}/modules/ssh_profile_setup.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/ssh_profile_setup.sh"
    
[ -f "${BASH_SOURCE[0]%/*}/modules/tailscale_helper.sh" ] \
    && source "${BASH_SOURCE[0]%/*}/modules/tailscale_helper.sh"

# ─────────────────────────────────────────────────────────────────────────────
# MENU ALIASES — quick access (defined in modules/dynamic_menu.sh)
# ─────────────────────────────────────────────────────────────────────────────
